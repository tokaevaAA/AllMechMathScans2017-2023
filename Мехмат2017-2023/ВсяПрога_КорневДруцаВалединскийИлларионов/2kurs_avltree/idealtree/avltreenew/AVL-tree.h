...
class Tree{
	...
	void klevpech(TreeNode<T>* root,int k,int currentlevel);
	...
	void Withdraw(const T&x){int found=Search(myroot, x);
				 if (found==-1) {printf("no element ");x.pechat();}
				 if (found==1) {printf("We are deleting "); x.pechat();int gr=0; myroot=Del(myroot,x,gr);}
				}
	...
	int Depth(){return Glubina(myroot);}
        ...
        int AVLTtest(TreeNode<T>* root, int n);
        void Test() { AVLTtest(myroot,0); }

	int Size(){return Razmer(myroot);}
	void klevelpechat(int k0){ int q= Glubina(myroot); if (q<k0) {printf("not deep enough for depth %d \n",k0);} 
							   else {printf("Deep enough for depth %d, here are members of %d level:\n",k0,k0);klevpech(myroot,k0,1);}
				 }
...
template<class T>
void Tree<T>::klevpech(TreeNode<T>* root,int k,int currentlevel){
    if (root==0) return;
    if (currentlevel==k) {root->pechat();}
    if (currentlevel<k){klevpech(root->left,k,currentlevel+1); klevpech(root->right,k,currentlevel+1);}
...
template<class T>
int Tree<T>::AVLTtest(TreeNode<T>* root, int n){
	int otv;
	n++;
	if (root==NULL) return 0;
	else otv= -Glubina(root->left)+Glubina(root->right);
	if (otv<-1 || otv>1) printf("balence error %d level %d\n",otv, n);
	n--;
	return otv;
