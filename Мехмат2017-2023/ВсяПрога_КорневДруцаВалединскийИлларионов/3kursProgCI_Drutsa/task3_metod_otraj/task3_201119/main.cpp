...
int  obratny_hod(int n, int k, double* a, double* e);
...
int metod_otraj_not_threaded(ARGS* pargs){
	...
	//if  (s<0.000000000000000001 && fabs(matrix(k,k))>= 0.000000000000000001) - then we are simply multiplying by edinichna matrix, it means nothing is needed to do
    ...
    //printf("Situation after pramou hod:\n");
    synchronize(pargs->total_threads);
    printf("thread_num=%d\n",pargs->thread_num);
    if (pargs->thread_num==1){printf("ggg_--\n"); print_matrix_rasshir(pargs->n,pargs->matrix,pargs->obr_matrix);}
    
    
    printf("matrix(n,n)=%le ; sravnili s nulem: %d \n",fabs(matrix(n,n)),fabs(matrix(n,n))<0.01);
    //if(fabs(matrix(n,n))<0.00000000000001){printf("Nety obr\n");}
    if(fabs(matrix(n,n))<0.00000000000001){printf("Nety obr\n"); *pargs->has_obr_matrix=-1; return -1;}
    
    //int flag=0; 
    if (pargs->thread_num==1){for (int k=n; k>0; k=k-1){obratny_hod(n,k,pargs->matrix,pargs->obr_matrix);}}
    
    //printf("flag=%d\n",flag);
    //if (flag==-1){*pargs->has_obr_matrix=-1;}
    return 0;
}
...
void* func_for_thread (void* pa)
{
  ...
  //printf("t_full_astron_in_thread=%f\n",t_full_astr_in_thread);
...
int obratny_hod(int n, int k, double* a, double* e){
    ...
    //print_matrix_rasshir(n,a,e);
    //if (a(n,n) < 0.00000000000001){
	//printf("Net obratnoy: obr_hod\n");
        //return -1 ;}
...
//				    udalenie(threads, args, matrix, obr_matrix, stolb); return -1;}
  ...
  if (has_obr_matrix != 1){printf("NET OBROTNOY!!!!\n");}
