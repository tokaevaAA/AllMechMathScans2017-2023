...
void ue_matrix_mult(int n, int k, double* e, double* u, double* anew){
    ...
    //printf("ue_matrix umnoj anew  ends\n");
    for (i=1; i<n+1; i=i+1){
	for (j=1; j<n+1; j=j+1){
            e(i,j)=anew(i,j);
	}
    }
    //printf("ue_matrix ends\n");
}
...
double nevazka(int n, double* a, double* e, double* anew){
    ue_matrix_mult(n,1,e,a,anew);
    printf("---------------------------\n");
    pechat_matrix(n,e);
    for (int i=1; i<n+1; i=i+1){e(i,i)=e(i,i)-1;}
    pechat_matrix(n,e);
    
    double tmp=0; double tekmax=-100000000;
    for (int i=1; i<n+1; i=i+1){
	tmp=0;
	for (int j=1; j<n+1; j=j+1){
            tmp=tmp+e(i,j);
